  ...
  if ($terms && count($terms) > 0) {
    ...
    foreach ($terms as $tier) {
      ...
      if (count($sponsors) > 0) {
        ...
        $output .= '<h3><i class="fa fa-trophy"></i> ' . $tier->name . ' Sponsors</h3>';
        ...
        $output .= '</div>';
        $output .= '</div>';
        $output .= '<div class="col-md-12 sponsors_container">';
        ...
        if ($sponsors && count($sponsors) > 0) {
          foreach ($sponsors as $sponsor) {
            ...
            $these_options = array('class' => 'sponsor' . $sponsor->ID, 'alt' => $sponsor->post_title);
